         ...
         if(rs.next())
            {
               ...
               sql="update  Purchase set Quantity=Quantity+"+Quantity+" where PurId='"+PurId+"'"; 
               nResult = Stmt.executeUpdate(sql);
  	           sql = "SELECT ProId FROM Product WHERE ProId='"+ProId+"'";
  	           rs = Stmt.executeQuery( sql ); 
  	          if(!rs.next())
  	           {
  	           sql="insert into Product(ProId,ProName,TypeId,ProUnit,NowPrice,ProPrice,Quantity,Memo)values('"
  	           +ProId+"','"+ProName+"','1','个',0,"+Price+",'"+Quantity+"','"+Memo+"')"; 
               nResult = Stmt.executeUpdate(sql);
  	           	}
  	           	else
  	           	{
  	           	 sql="update  Product set Quantity=Quantity+"+Quantity+" where ProId='"+ProId+"'"; 
  	           	 nResult = Stmt.executeUpdate(sql);	
  	           		}
            }
         else
